...
class HybridThermodynamicRBM:
    ...
    def ensemble_diagnostics(
        ...
        # Execute Log-lag for steps 10, 20, 30 and 40. If the
        # 'MCMC Mix Index' outputs are decreasing,
        # the process is finalized.
        lag_values = torch.unique(
            torch.logspace(0, math.log10(max_lag), steps=18, device=device).long()
        )
...
if __name__ == "__main__":
    ...
    for result in list(results):
        ...
        entropy_low = result['entropy_spatial_std']
        entropy_high = result['pixel_entropy']
        in_band = entropy_low <= result['mcmc_mix_index'] <= entropy_high

        print(f"Entropy Band             : Closed interval [{entropy_low:.6f}, {entropy_high:.6f}]")
        print(
            f"Choice(Mix | Band)       : C({result['mcmc_mix_index']:.6f} | [{entropy_low:.6f}, {entropy_high:.6f}]) = {result['mcmc_mix_index']:.6f}")
