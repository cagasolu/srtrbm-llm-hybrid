...
class HybridThermodynamicRBM:
    ...
    def __init__(
        ...
        self.yablo_expectation = []
        self.yablo_violation = []
    ...
    def train(self, data, energy_tracker=None, client=None):
        ...
        for epoch_goal in epoch_bar:
            ...
            if len(self.F_gap_hist) > 30:
                window_standard = min(30, len(self.F_gap_hist))
                energy_window = self.F_gap_hist[-window_standard:]
                std_val = float(np.std(energy_window))

                delta_w_recent = self.delta_w_hist[-1]
                stagnation = (std_val < 0.01) and (delta_w_recent < 1e-4)

                periodic_check = (epoch_goal % 60 == 0)

                trigger_llm = (
                        stagnation
                        or periodic_check
                        or (std_val < 0.008 and delta_w_recent < 5e-5)
                )

                if trigger_llm:
                    gap_global = float(self.F_gap_hist[-1] - self.F_gap_hist[0])
                    entropy_global = float(self.persistent_div_hist[-1] - self.persistent_div_hist[0])
                    temp_global = float(self.temp_hist[-1] - self.temp_hist[0])
                    beta_global = float(self.beta_eff_hist[-1] - self.beta_eff_hist[0])

                    gap_local = float(self.F_gap_hist[-1] - self.F_gap_hist[-window_standard])
                    entropy_local = float(self.persistent_div_hist[-1] - self.persistent_div_hist[-window_standard])
                    temp_local = float(self.temp_hist[-1] - self.temp_hist[-window_standard])
                    beta_local = float(self.beta_eff_hist[-1] - self.beta_eff_hist[-window_standard])

                    collapse_ordered = (
                            current_T.item() < 0.15 and
                            beta_eff > 60 and
                            delta_w_recent < 1e-5
                    )

                    collapse_disordered = (
                            current_T.item() > 5.0 and
                            beta_eff < 1.0 and
                            abs(gap_local) < 1e-4
                    )
                    ...
                    true_stagnation = (
                            std_val < 0.005 and
                            delta_w_recent < 1e-5 and
                            abs(gap_local) < 1e-4
                    )

                    if collapse_ordered or collapse_disordered or true_stagnation:
                        force_flag = True
                    else:
                        force_flag = False
                    ...
                    history = {
                        ...
                        "gap_std": float(np.std(self.F_gap_hist)),
                        "entropy_std": float(np.std(self.persistent_div_hist)),
                        "temp_std": float(np.std(self.temp_hist)),
                        "delta_w": float(delta_w_recent),
                        "stagnation": bool(stagnation),
                        "learning_active": bool(delta_w_recent > 1e-4),
                    }

                    metrics = {
                        "entropy": float(self.persistent_div_hist[-1]),
                        "diversity": float(self.persistent_div_hist[-1]),
                        "energy_gap": float(self.F_gap_hist[-1]),
                        "std": std_val,
                        "temperature": float(current_T.item()),
                        "flip_rate": float(flip_epoch.item()),
                        "delta_w": float(delta_w_recent),
                    ...
                    trend = {
                        "energy_gap_slope": gap_local,
                        "entropy_slope": entropy_local,
                        "temperature_slope": temp_local,
                        "beta_slope": beta_local,
                    }

                    print(f"\n\n[LLM TRIGGER] epoch={epoch_goal} | std={std_val:.6f} | stagnation={stagnation}")

                    # DEBUG: forced stagnation scenario

                    # metrics["history"]["stagnation"] = True
                    # metrics["history"]["learning_active"] = False
                    # trend["energy_gap_slope"] = 0.0

                    # stagnation = True

                    conclusion = llm_determine(metrics, trend, client=client)

                    ctrl = conclusion["epistemic"]
                    confidence = conclusion.get("confidence", 0.0)

                    confidence = max(0.0, min(1.0, confidence))

                    raw_apply = ctrl["apply"]

                    apply_control = raw_apply and (confidence >= self.momentum)

                    recent_apply_rate = (
                        sum(self.yablo_violation) / len(self.yablo_violation)
                        if len(self.yablo_violation) > 0 else 0.0
                    )

                    expected_no_intervention = 1.0 - recent_apply_rate

                    self.yablo_expectation.append(expected_no_intervention)
                    self.yablo_expectation = self.yablo_expectation[-5:]

                    violation = 1 if (apply_control and expected_no_intervention > 0.5) else 0

                    self.yablo_violation.append(violation)
                    self.yablo_violation = self.yablo_violation[-5:]

                    yablo_pressure = (
                        sum(v * e for v, e in zip(self.yablo_violation, self.yablo_expectation))
                        / len(self.yablo_violation)
                    )

                    if yablo_pressure > 0.6:
                        print("[YABLO] Self-contradiction detected → forcing exploration")
                        self.log_temperature += 0.08
                        self.gibbs_steps = min(5, self.gibbs_steps + 1)

                    if apply_control:
                        s = ctrl["strength"]
                        actions = ctrl["actions"]

                        if actions.get("increase_gibbs_steps"):
                            self.gibbs_steps = max(1, min(5, int(1 + s * 4)))

                        if actions.get("increase_temperature"):
                            delta_T = conclusion["intensity"]["temperature"]
                            self.log_temperature += 0.05 * max(0.0, delta_T)

                    print("\n[LLM RAW]")
                    print(conclusion)

                    if raw_apply and not apply_control:
                        print(f"[FILTER] apply=True but REJECTED | conf={confidence:.3f} < momentum={self.momentum:.2f}")

                    elif apply_control:
                        print(f"[FILTER] apply=True and ACCEPTED | conf={confidence:.3f}")

                    else:
                        print(f"[FILTER] apply=False | no intervention")

                    print()

                    decision = conclusion.get("decision", "continue")
                    confidence = conclusion.get("confidence", 0.0)
                    analysis_text = conclusion.get("analysis", None)

                    if analysis_text:
                        analysis_signal = ANASIS(analysis_text)
                        analysis_weighted = analysis_signal * confidence
                        self.analysis_history.append(analysis_weighted)

                        print(f"[ANALYSIS SIGNAL] {analysis_weighted:.3f}")

                    if force_flag:
                        print("[SYSTEM WARNING] Critical regime detected")

                    print(f"[LLM DECISION] {decision} | confidence={confidence:.3f}")

                    if len(self.analysis_history) >= 3:
                        window = self.analysis_history[-3:]
                        avg_signal = sum(window) / 3
                        strong_votes = sum(1 for x in window if x > 0.5)

                        if avg_signal > 0.5 and strong_votes >= 2:
                            print("[SYSTEM] Persistent failure pattern detected (analysis)")
                            return "ABORT"

                    if decision == "abort_and_restart":

                        if confidence > 0.9:
                            print("[LLM] Hard abort")
                            return "ABORT"

                        elif confidence >= 0.75 and stagnation:
                            self.abort_counter += 1
                            print(f"[LLM] Soft abort signal ({self.abort_counter}/3)")

                            if self.abort_counter >= 3:
                                print("[LLM] Confirmed abort (3 consecutive signals)")
                                return "ABORT"

                        else:
                            print("[LLM] Abort rejected")
    ...
    def generate_ensemble_samples(
        ...
        for to in range(steps):

            # Temperature schedule:

            if to < final_cool_start and final_cool_start > 0:
                anneal_factor = 1.0 - (to / final_cool_start)
                noise_scale = temp_noise * anneal_factor

                noise = torch.randn(n_chains, 1, device=device)

                T = T_base * torch.exp(noise_scale * noise)
            else:
                T = T_base
