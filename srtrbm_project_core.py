...
class HybridThermodynamicRBM:
    ...
    def __init__(
        ...
        self.myra_plastic_index = pow((1/2 + math.sqrt(23/108)), 1/3) + pow((1/2 - math.sqrt(23/108)), 1/3)
    ...
    def llm_uncertainty_refinement(self, state, client=None):
        ...
        print(f"\n[DEBUG] LLM refinement (FINAL GUIDED) | B={B}, D={D}")

        x_out = state.clone()

        def to_ascii(grid_2d):
            return "\n".join(
                "".join("#" if v > 0.5 else "." for v in row)
                for row in grid_2d
            )

        def apply_point(x_vec, row, col, radius_value, value):
            for dx in range(-radius_value, radius_value + 1):
                for dy in range(-radius_value, radius_value + 1):
                    nx, ny = row + dx, col + dy
                    if 0 <= nx < 28 and 0 <= ny < 28:
                        x_vec[nx * 28 + ny] = value

        energies = self.free_energy(x_out, T)

        scores = (energies - energies.mean()) / (energies.std() + 1e-8)
        high_energy_ratio = (scores > 1.0).float().mean().item()
        k = math.ceil((self.myra_plastic_index / 100) * B * (1 + high_energy_ratio))
        _, selected_indices = torch.topk(scores, k=min(k, B))

        print(f"[DEBUG] LLM will run on {len(selected_indices)} samples")

        total_accept = 0

        for b in selected_indices:
            b = int(b.item())
            print(f"[LLM CALL] sample={b}")

            grid_img = x_out[b].view(28, 28)

            response = LLMRefinementSignal({
                "image_ascii": to_ascii(grid_img)
            }, client=client)

            if not isinstance(response, dict):
                continue

            points_add = response.get("points_add", [])
            points_remove = response.get("points_remove", [])
            llm_conf = float(response.get("confidence", 0.0))

            if not points_add and not points_remove:
                continue
            ...
            for (r, command) in points_add:
                apply_point(new_x, r, command, radius, 1.0)

            for (r, command) in points_remove:
                apply_point(new_x, r, command, radius, 0.0)

            F_old = self.free_energy(old_x.unsqueeze(0), T)[0]
            F_new = self.free_energy(new_x.unsqueeze(0), T)[0]

            delta_E = F_new - F_old

            delta_E_scaled = delta_E / (T * 50.0 + 1e-8)

            conf = llm_conf

            accept_prob = torch.sigmoid(
                -delta_E_scaled - 4.0 * (conf - 1.0)
            )

            accept = any(
                (torch.rand(1).item() / self.golden_ratio) < accept_prob.item()
                for _ in range(3)
            )

            print(
                f"[ENERGY] ΔE={delta_E.item():.2f} | "
                f"scaled={delta_E_scaled.item():.2f} | "
                f"conf={llm_conf:.2f} | "
                f"P={accept_prob.item():.3f}"
            )
            print(f"[ACCEPT] {accept}")
            print(f"[STRUCTURAL EFFECT] add={len(points_add)}, remove={len(points_remove)}")
        ...
        print("\n[LLM PIPELINE STATS]")
        print(f"accepted samples: {total_accept}/{len(selected_indices)}")
